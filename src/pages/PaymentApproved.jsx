...
import { getApprovedPayments, getUserDetails } from '../lib/apiEndPoints';
...
const PaymentApproved = () => {
	...
	useEffect(() => {
		async function fetch() {
			setLoading(true);
			const { requestSucessful, payments } = await getApprovedPayments();
			if (requestSucessful) {
		...
		<>
			<div className='border-[1px] rounded-xl border-gray-900 mx-auto md:w-[95%] py-10'>
				<div className='flex justify-center'>
					<InputFilter action={'Approved Requests'} filter={false} />
				</div>
				{loading ? (
