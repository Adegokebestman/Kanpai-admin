...
const SupplierProductsView = () => {
  ...
  const columns = [
    ...
    {
      name: "Unit Price",
      selector: (row) => <h1 className="text-sm md:text-base">{row.amount}</h1>,
    },
    ...
    },
    {
      ...
      selector: () => (
        <div className="flex items-center gap-3">
          <span
            className="p-2 rounded-full bg-gray-100 hover:bg-white flex items-center justify-center"
            onClick={() => setIsEditModalOpen(true)}
          >
            <EditIcon className="cursor-pointer" />
          </span>
          <span className="p-2 rounded-full bg-gray-100 hover:bg-white flex items-center justify-center">
            <DeleteIcon className="cursor-pointer" onClick={() => setOpenDelete(true)} />
          </span>
        </div>
      ),
    },
